...
### ② 신발 이름 — `data/shoes.csv`

제가 사진을 보고 추측해서 넣어둔 이름이라 **거의 틀렸을 겁니다.** 고쳐주세요.

| id | 지금 적어둔 이름 | 어느 사진 |
|---|---|---|
| `shoe-01` | NIKE 페가수스 41 | `shoe-01.jpg` 전체 |
| `shoe-02` | PUMA 디비에이트 나이트로 3 | `shoe-02.jpg` **왼쪽 아래** 크림색 |
| `shoe-03` | ON 클라우드몬스터 2 | `shoe-02.jpg` **오른쪽 위** 하늘색 |

> 신발 사진은 2장인데 3켤레가 찍혀 있어서, `shoe-02.jpg` 한 장을 두 칸으로 잘라 씁니다.
> `크롭` 칸의 `x,y,너비,높이`는 원본 대비 %입니다. 잘린 위치가 어색하면 이 숫자를 조절하세요.
