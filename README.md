...
- 상단 누적 거리는 **월별 기록 합계 220.2km** (2026.01.01 — 2026.08.23)
- 지도는 GPX 가 남아있는 **서울 9회 / 74.3km / 7개 코스**
- 메달 8개 (2021 — 2026), 신발 3켤레
- 만족도·한줄평은 대회 1건 빼고 **비어 있습니다.** 아래 ①을 채워주세요.
...
2026-04-05 대회를 빼고 만족도·한줄평은 비워두었습니다.

### ②-1 코스 이름과 설명 — `data/courses.csv`

| 칸 | 설명 |
|---|---|
| `대표날짜` | 그 코스에서 **가장 길게 달린 날**. 이 날짜로 코스를 찾습니다 |
| `코스명` | 지도·목록·툴팁에 나오는 이름 |
| `설명` | 툴팁에서 이름 아래 나오는 코스 소개. 비워도 됩니다 |

빌드할 때 `courses.csv` 에 없는 대표날짜가 있으면 `코스 1` 같은 임시 이름과 함께
경고를 찍습니다. 그 날짜로 행을 추가하면 됩니다.

### ②-2 월별 거리 — `data/monthly.csv`

맨 위 큰 숫자와 월별 막대가 이 파일에서 나옵니다. GPX 와 무관하게 직접 적습니다.

| 칸 | 설명 |
|---|---|
| `연월` | `2026-01` 형식 |
| `거리km` | 그 달에 달린 총 거리 |
| `기준일` | 마지막 달에만 적습니다. 집계 종료일로 표시됩니다 |

> GPX 는 일부 러닝만 남아있어서, 위 합계(220.2km)와 지도에 그려진 거리(74.3km)가
> 다릅니다. 히어로 아래에 그 차이를 설명하는 문구가 자동으로 붙습니다.
...
### ⑥ 대청호 훈련 — `data/training.csv`

`상태` 칸이 이 섹션의 전부입니다. 훈련을 마치면 `완료`, 걸렀으면 `미진행`,
아직이면 비워둡니다. `실제km` 를 적으면 계획과 다른 만큼이 그래프에 나타납니다.

| 칸 | 설명 |
|---|---|
| `주차` / `시작일` / `종료일` / `주제` | 주차 카드 묶음. 오늘이 낀 주가 '이번 주'로 강조됩니다 |
| `종류` | `이지런` `롱런` `템포런` `언덕` `롤링힐` `목표페이스` `대회`. 배지 색이 달라집니다 |
| `계획km` / `실제km` | 롱런·대회는 성장 곡선에 점으로 찍힙니다 |
| `상태` | `완료` / `미진행` / 빈칸 |

대회 정보와 목표는 `data/race.csv`, 코스 고도는 `data/race_profile.csv` 입니다.
D-day 는 `race.csv` 의 `일시` 에서 매일 자동 계산됩니다.

> **CSV에 큰따옴표(`"`)를 쓰지 마세요.** `7'00"` 처럼 적으면 파서가 인용
> 시작으로 읽어 그 뒤 내용이 전부 한 칸으로 뭉갑니다. 페이스는 프라임 기호
> `7′00″` (′ = U+2032, ″ = U+2033)로 적어두었습니다.

---

## 고치면 좋을 것

- **2026.03.21 일산 호수공원** — 서울이 아니라 지도에서 빠져 있습니다
  (히어로 하단에 안내 문구가 뜹니다).
- **특정 러닝을 기록에서 빼려면** `build/build.py` 상단 `EXCLUDED_DATES` 에
  날짜를 추가하세요. GPX 원본은 `map/` 에 그대로 남아서 언제든 되살릴 수 있습니다.
  지금은 `2026-08-01`, `2026-08-23` (종로 — 성북 언덕)이 빠져 있습니다.
...
data/runs.csv           ← ① 후기
data/shoes.csv          ← ③ 신발
data/courses.csv        ← ②-1 코스 이름·설명
data/monthly.csv        ← ②-2 월별 거리
data/medals.csv         ← ⑤ 메달
data/training.csv       ← ⑥ 대청호 훈련 계획
...
트랙이 지나간 60m 격자를 비교합니다. 두 가지 기준을 씁니다.

- **Jaccard 유사도 0.40 이상** — 대체로 같은 길을 달린 경우
- **포함률 85% 이상 + 길이비 35% 이상** — 같은 곳에서 출발해 점점 멀리
  다녀오는 경우. 길이 차가 크면 Jaccard 가 낮게 나와서(5km 와 12km 가 0.25)
  이 기준이 따로 필요합니다. 길이비 조건은 동네에서 잠깐 뛴 2km 러닝이
  같은 길을 지난다는 이유로 8km 코스에 흡수되는 걸 막습니다.

한 코스로 묶이면 지도에 **모든 트랙을 겹쳐** 그립니다. 긴 것부터 옅게 깔고
짧은 것을 위에 밝게 얹어서, 여러 번 지난 공통 구간이 가장 밝게 남습니다.
가장 긴 쪽이 짧은 쪽의 1.5배를 넘으면 툴팁에 '단계' 목록이 붙는데,
단계 이름은 `data/courses.csv` 의 `단계` 칸에 `|` 로 구분해 적습니다.

임계값은 `build/build.py` 의 `SIMILARITY_THRESHOLD`, `CONTAINMENT_THRESHOLD`,
`MIN_LENGTH_RATIO`.
